        ...
        public void SkipToken()
        {
            switch (CurrentToken)
            {
                ...
                case Token.ApplicationExtension:
                    ...
                    break;
                default: throw new InvalidOperationException(string.Format("Cannot skip token {0}", CurrentToken));
            }
        }
        ...
        public GifGraphicControl ReadGraphicsControl()
        {
            ...
            switch (disposalMethodValue)
            {
                ...
                case 3:
                    ...
                    break;
                default:
                    throw new ArgumentException(string.Format("Invalid disposal method type: {0}",
                        disposalMethodValue));
            }
        ...
        private void AssertToken(Token token)
        {
            if (CurrentToken != token)
                throw new InvalidOperationException(
                    string.Format("Cannot invoke this method while current token is \"{0}\", ", CurrentToken) +
                    string.Format("method should be called when token is {0}", token));
        }
