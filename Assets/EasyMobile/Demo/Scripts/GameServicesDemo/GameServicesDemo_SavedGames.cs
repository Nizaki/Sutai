...
namespace EasyMobile.Demo
{
    ...
    {
#if EASY_MOBILE_PRO
        [Header("Object References")] public DemoUtils demoUtils;
        public GameObject scrollableListPrefab;

        [Header("Saved Games Demo")] public GameObject isSavedGamesEnabledInfo;
        ...
        [Tooltip(
            "Check to enable manual conflict resolution. The implemented policy is pick the saved game with biggest stored value.")]
        ...
        public class DemoSavedGameData
        {
            ...
            public DemoSavedGameData(int data)
            {
                ...
                for (var i = 0; i < TEST_DATA_BYTE_COUNT; i++) largeData[i] = 0xAA;
            }
        }

        private const int TEST_DATA_BYTE_COUNT = 10 * 1024;
        ...
        private void Start()
        {
            ...
            if (!resolveConflictsManually)
                demoUtils.DisplayBool(autoConflictResolutionInfo, true,
                    ...
                    EM_Settings.GameServices.AutoConflictResolutionStrategy.ToString());
            else
        ...
        public void OpenSavedGame()
        {
            ...
            {
                var name = savedGameNameInput.text;
        ...
        public void FetchAllSavedGames()
        {
            GameServices.SavedGames.FetchAllSavedGames((SavedGame[] games, string error) =>
            {
                if (string.IsNullOrEmpty(error))
                {
                    ...
                    foreach (var game in allSavedGames)
                    {
#if UNITY_IOS
        ...
        public void ReadSavedGame()
        {
            if (selectedSavedGame == null)
            {
                NativeUI.Alert("Alert",
                    "Please select a saved game by using the select UI or entering its name into the input box.");
                return;
            }
            else
            {
                if (selectedSavedGame.IsOpen)
                    ...
                    DoOpenSavedGame(selectedSavedGame.Name,
                        (SavedGame game, string error) =>
                        {
                            ...
                            {
                                NativeUI.Alert("Alert",
                                    "Couldn't read saved game data because it's failed to open with error: " + error);
                            }
                        });
            }
        }
        ...
        public void WriteSavedGame()
        {
            ...
            {
                // Get the input data
                var value = Convert.ToInt32(savedGameDataInput.text);
                var data = SavedGameDataToByteArray(new DemoSavedGameData(value));
                ...
                if (selectedSavedGame.IsOpen)
                    ...
                    DoOpenSavedGame(selectedSavedGame.Name,
                        (SavedGame game, string error) =>
                        {
                            ...
                            {
                                NativeUI.Alert("Alert",
                                    "Couldn't write saved game data because it's failed to open with error: " + error);
                            }
                        });
            }
        }
            ...
            if (!resolveConflictsManually)
                ...
                GameServices.SavedGames.OpenWithManualConflictResolution(name, true,
                    (baseGame, baseData, remoteGame, remoteData) =>
                    {
                        ...
                        return baseDataObj.demoInt >= remoteDataObj.demoInt
                            ? SavedGameConflictResolutionStrategy.UseBase
                            : SavedGameConflictResolutionStrategy.UseRemote;
                    },
                    callback);
        }
        ...
        private void OnSavedGameRead(SavedGame game, byte[] data, string error)
        {
            ...
            else
            {
                ...
                if (savedData != null)
                    NativeUI.Alert("Saved Game Data Retrieved",
                        "The data of saved game '" + game.Name + "' contains value: " + savedData.demoInt);
                else
                    NativeUI.Alert("Alert", "Saved game '" + game.Name + "' has no data!");
            }
        }

        private DemoSavedGameData ByteArrayToSavedGameData(byte[] data)
        {
            if (data != null)
            {
                ...
                var savedData = JsonUtility.FromJson<DemoSavedGameData>(jsonStr);
        ...
        private byte[] SavedGameDataToByteArray(DemoSavedGameData dataObj)
        {
            if (dataObj != null)
            {
                // Convert to json string
                var jsonStr = JsonUtility.ToJson(dataObj);
