...
#pragma warning disable 0414 // restore value not used warning
            ...
            public void Update(Texture2D avatar, Sprite mark, Color markColor, string name, bool hightLight,
        ...
        [SerializeField] private Color xColor = Color.white, oColor = Color.black;

        [SerializeField] [Range(0, 1)] private float spaceRate = 0.1f;

        [SerializeField] [Range(1, 100)] private float progressUISpeed = 1f, progressUIDisappearTime = 3f;
        ...
        [SerializeField] private AlertPopup popup;

        [SerializeField] private GameObject gameSection = null,
        ...
        [SerializeField] private RectTransform gridRect = null;

        [SerializeField] private Button showMatchesUIButton = null,
            hideGameButton = null,
            leaveMatchButton = null,
            rematchButton = null;

        [SerializeField] private Button tilePrefab = null;

        [SerializeField] private Text progressText = null;

        [SerializeField] private Dropdown gameSizeSelector = null;

        [SerializeField] private Sprite blankImage = null, xImage = null, oImage = null;

        [SerializeField] private PlayerInfosUI selfInfos, opponentInfos;

        [SerializeField] private Texture2D defaulAvatar = null;

        public float TileSize { get; private set; }

        public float TileSpacing => TileSize * spaceRate;
        ...
        {
            control.Move(x, y, mark => { UpdateTileUI(x, y, mark.ToTileState()); });
        }
        ...
        public void ShowGameOverUI(MatchOutcome.ParticipantResult result, bool showRematchButton)
        {
            ...
            opponentInfos.Update(
                tempModel.Opponent != null && tempModel.Opponent.Player != null
                    ? tempModel.Opponent.Player.image
                    : defaulAvatar,
        ...
        public void StopProgressUI(string message, float appearTime = -1)
        {
            ...
            progressUIStoppingCoroutine =
                StartCoroutine(ProgressUIStoppingCoroutine(message,
                    appearTime < 0 ? progressUIDisappearTime : appearTime));
        }
        ...
        private IEnumerator ProgressRunningUICoroutine(string message)
        {
            ...
            while (true)
            {
                var newMessage = message + GetDotMessageForProgessUI(loopTimes);
                progressText.text = newMessage;

                loopTimes++;
                yield return
                    new WaitForSeconds(50 /
                                       progressUISpeed); /// 100 = <see cref="progressUISpeed"/> max range value / 2.
            }
        }
        ...
        private void InstantiateBoard(TicTacToeGameModel model)
        {
            ...
            {
                for (var i = 0; i < model.TransferDatas.Board.Length; i++)
                {
                    var innerArray = model.TransferDatas.Board[i];
                    for (var j = 0; j < innerArray.Length; j++) UpdateTileUI(i, j, innerArray[j]);
                }
        ...
        {
            var gridSize = gridRect.sizeDelta.x;
            var sizeWithoutSpace = gridSize / boardSize;
