...
namespace EasyMobile.Demo
{
    public class Racing3DGameControl : GameServicesDemo_Multiplayer_BaseControl, IRealTimeMultiplayerListener
    {
        ...
        [SerializeField] private Racing3DGameView view = null;

        [SerializeField] private Racing3DCameraControl cameraControl = null;

        [SerializeField] private Racing3DCarControl hostCar = null, guestCar = null;

        [SerializeField] private Transform roadTransform = null;

        [SerializeField] private GameObject powerUpPrefab = null;

        [SerializeField] private GameObject[] sideObjects = null;

        [SerializeField] private Texture2D anonymousAvatar = null;

#if UNITY_ANDROID
        [SerializeField] private float readyMessageDelay = -0.5f;
#else
        private float readyMessageDelay = 0f;
#endif

        [Header("Send Start Message")] [SerializeField]
        private float startMessageInterval = 10f;

        [SerializeField] private int startMessageLimit = 5;

        [Header("PowerUps")] [SerializeField] private string powerUpName = "Nitro";

        [SerializeField] [Range(0f, 1f)] private float powerUpsFrequency = 0.5f;

        [SerializeField] private float powerUpsZ = -0.5f;

        [SerializeField] private Vector2 powerUpsYRange = Vector2.one, powerUpsDistanceRange = Vector2.one;

        [SerializeField] private List<float> powerUpsX = new List<float>();

        [Header("Side Objects")] [SerializeField]
        private Vector2 sideObjectsYRange = Vector2.one, sideObjectsDistanceRange = Vector2.one;

        [SerializeField] private float sideObjectMaxDeltaX = 0.5f;

        [SerializeField] private float sideObjectMaxDeltaY = 1f;

        [SerializeField] private List<float> sideObjectsX = new List<float>();

        [Header("Reliable Settings")] [SerializeField]
        private bool move = true;

        [SerializeField] private bool hitPowerUp = true,
            useNitro = true,
            finish = true,
            start = true,
            ready = true,
        ...
        public bool IsRoomConnected => GameServices.RealTime.IsRoomConnected();
        ...
        {
            if (model == null || opponentId == null || model.PowerUpsPosition == null ||
                model.SideObjectsPosition == null)
                return;
        ...
        public void StartGame()
        {
            ...
            var opponent = GameServices.RealTime.GetConnectedParticipants().Where(p => !p.ParticipantId.Equals(selfId))
                .FirstOrDefault();
        ...
        private void InstantiateSideObjects(List<Vector3> positions)
        {
            foreach (var position in positions)
            {
                var randX = Random.Range(-sideObjectMaxDeltaX, sideObjectMaxDeltaX);
                var randY = Random.Range(-sideObjectMaxDeltaY, sideObjectMaxDeltaY);
                var i = Random.Range(0, sideObjects.Length);
                var sObject = Instantiate(sideObjects[i], position + new Vector3(randX, randY, 0), Quaternion.identity,
                    roadTransform);
        ...
        {
            var generatedPosition = new List<Vector3>();

            var currentY = powerUpsYRange.x;
            while (currentY < powerUpsYRange.y - powerUpsDistanceRange.y)
            {
                var newPowerUpsCount = Random.Range(1, powerUpsX.Count);
                var currentXs = powerUpsX.OrderBy(_ => Random.value).Take(newPowerUpsCount).ToList();
                var yDistance = Random.Range(powerUpsDistanceRange.x, powerUpsDistanceRange.y);
                currentY += yDistance;

                foreach (var x in currentXs)
                {
        ...
        private List<Vector3> GenerateRandomSidePositions()
        {
            ...
            var currentY = sideObjectsYRange.x;
            while (currentY < sideObjectsYRange.y - sideObjectsDistanceRange.y)
            {
                var newObjectsCount = Random.Range(1, powerUpsX.Count);
                var currentXs = sideObjectsX.OrderBy(_ => Random.value).Take(newObjectsCount).ToList();
                var yDistance = Random.Range(sideObjectsDistanceRange.x, sideObjectsDistanceRange.y);
                currentY += yDistance;
        ...
        private IEnumerator SendStartMessageCoroutine()
        {
            ...
            while (!isStartMessageReceived)
                ...
                {
                    var title = "No Reponse";
                    var message =
                        "There is no reponse from your opponent. You can click the \"Back\" button in the header to leave the match.";
